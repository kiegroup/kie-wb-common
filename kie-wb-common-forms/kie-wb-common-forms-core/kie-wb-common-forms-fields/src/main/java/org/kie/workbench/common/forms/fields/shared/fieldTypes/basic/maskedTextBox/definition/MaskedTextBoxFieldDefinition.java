...
public class MaskedTextBoxFieldDefinition extends AbstractMaskedTextBoxFieldDefinition implements HasMaxLength {
    ...
    /**
     * Validates that minLength is smaller than or equal to maxLength
     */
    @AssertTrue(message = "Min length must be smaller than or equal to max length")
    public boolean isMinLengthValidAgainstMaxLength() {
        if (getMinLength() != null && maxLength != null) {
            return getMinLength() <= maxLength;
        }
        return true;
    }

    /**
     * Validates that maskingFromStartLength is not greater than maxLength
     */
    @AssertTrue(message = "Masking from start length cannot be greater than max length")
    public boolean isMaskingFromStartLengthValidAgainstMaxLength() {
        if (getMaskingFromStartLength() != null && maxLength != null) {
            return getMaskingFromStartLength() <= maxLength;
        }
        return true;
    }
