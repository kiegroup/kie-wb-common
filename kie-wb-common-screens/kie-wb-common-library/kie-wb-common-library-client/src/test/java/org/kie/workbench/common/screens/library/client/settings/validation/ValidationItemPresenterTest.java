...
public class ValidationItemPresenterTest {
    ...
    public void testSetup() {
        ...
        final ProjectRepositories.ProjectRepository projectRepository = mock(ProjectRepositories.ProjectRepository.class);
        final ValidationPresenter parentPresenter = mock(ValidationPresenter.class);

        doReturn(new MavenRepositoryMetadata("test", "https://test.url", MavenRepositorySource.LOCAL))
                .when(projectRepository).getMetadata();

        validationItemPresenter.setup(projectRepository, parentPresenter);
