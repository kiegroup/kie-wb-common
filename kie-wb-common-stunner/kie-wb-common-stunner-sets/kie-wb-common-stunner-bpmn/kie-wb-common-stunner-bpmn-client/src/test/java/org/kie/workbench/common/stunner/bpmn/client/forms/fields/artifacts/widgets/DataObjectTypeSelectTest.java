...
public class DataObjectTypeSelectTest extends ReflectionUtilsTest {
    ...
    public void setValue() {
        ...
        tested.setValue("some");
        verify(tested).checkIfNotExists("some");
        assertTrue(tested.getValue().equals("some"));
    ...
    @Test
    public void onKeyupEnterNotEnter() {
        setFieldValue(tested, "currentAction", DataObjectTypeSelect.Mode.NONE);
        select.style = new CSSStyleDeclaration();
        addOrEdit.style = new CSSStyleDeclaration();
        KeyboardEvent event = spy(new KeyboardEvent("keyup"));
        ...
        event = spy(new KeyboardEvent("keyup"));
        event.key = "Escape";
        addOrEdit.value = "otherValue";
        setFieldValue(tested, "selectedValue", "value");
        ...
        listener = event1 -> tested.onKeyup(event1);
        listener.handleEvent(event);
        ...
        option = spy(HTMLOptionElement.class);
        option.text = "";
        option.value = "";
        select.options.length = 1;
        when(collection.item(anyInt())).thenReturn(option);
