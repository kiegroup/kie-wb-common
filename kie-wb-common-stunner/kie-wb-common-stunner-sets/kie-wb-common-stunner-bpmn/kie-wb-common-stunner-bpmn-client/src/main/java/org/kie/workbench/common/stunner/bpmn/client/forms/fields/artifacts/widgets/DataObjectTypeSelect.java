...
public class DataObjectTypeSelect extends Composite implements HasValue<String> {
    ...
    @PostConstruct
    public void init() {
        addDataTypeToSelect("", "", Mode.ADD, false, false, true);
        addDataTypeToSelect(CUSTOM_PROMPT, CUSTOM_PROMPT, Mode.ADD, true, false, false);
        ...
        if (validateValue(value)) {
            if (!currentAction.equals(Mode.NONE)) {
                if (currentAction.equals(Mode.ADD)) {
                    ...
                    for (int i = select.options.length - 1; i >= 0; --i) {
    ...
    public void setValue(String value) {
        this.setValue(value.isEmpty() ? Object.class.getSimpleName() : value, false);
    }
