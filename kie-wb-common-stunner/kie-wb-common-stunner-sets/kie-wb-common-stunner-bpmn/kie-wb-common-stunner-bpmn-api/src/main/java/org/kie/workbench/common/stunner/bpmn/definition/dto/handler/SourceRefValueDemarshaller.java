...
public class SourceRefValueDemarshaller extends CustomXMLDeserializer<SourceRef> {

    @Override
    protected SourceRef doDeserialize(
            XMLReader reader, XMLDeserializationContext ctx, XMLDeserializerParameters params)
            throws XMLStreamException {
        SourceRef result = new SourceRef();
        reader.next();
        result.setValue(reader.nextValue());
        reader.next();
        return result;
    }

    @Override
    public SourceRef deserialize(
            String value, XMLDeserializationContext ctx, XMLDeserializerParameters params)
            throws XMLDeserializationException {
        return new SourceRef(value);
    }
}
