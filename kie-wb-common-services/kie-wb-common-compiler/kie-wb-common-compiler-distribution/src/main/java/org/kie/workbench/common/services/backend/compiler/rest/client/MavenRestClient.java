...
public class MavenRestClient {
    ...
    public CompletableFuture<KieCompilationResponse> call(String projectPath, String mavenRepo, String settingsXML, String url) {
        ...
        Future<?> future = ClientBuilder.newBuilder().build()
                .target(url)
                .request().header("project", projectPath).header("mavenrepo", mavenRepo).header("settings_xml",settingsXML)
